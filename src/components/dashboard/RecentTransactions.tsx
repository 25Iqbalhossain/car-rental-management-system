      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-3 border-b border-slate-100">
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-medium">Live rental booking status</p>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap justify-between sm:justify-end">
            ...
            <button onClick={() => setShowAll(true)} className="text-[11px] font-bold text-orange-500 hover:text-orange-600 flex items-center gap-0.5">
              <span>View All</span>
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-slate-100 text-slate-400 font-bold uppercase text-[9px]">
                ...
                <th className="py-2 px-2">Booking Details</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {transactions.slice(0, 4).map((tx, idx) => (
                <tr key={tx.id} className="hover:bg-slate-50/80 transition-colors">
                  <td className="py-2 px-2 text-[10px] font-bold text-slate-400">{idx + 1}</td>
                  <td className="py-2 px-2">
                    <div className="flex items-center gap-2">
                      <div className="relative w-8 h-8 rounded-md bg-slate-100 overflow-hidden border border-slate-200 shrink-0">
                        <Image src={tx.vehicleImage} alt={tx.vehicleName} fill className="object-cover" sizes="32px" />
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-xs text-slate-800 truncate">{tx.vehicleName}</p>
                        <p className="text-[9px] text-slate-400 font-medium">{tx.timeAgo}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-2 px-2 text-right font-black text-slate-900 text-xs">£{tx.amount.toFixed(2)}</td>
                </tr>
              ))}
              {transactions.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-6 text-center text-xs text-slate-400">No transactions found.</td>
                </tr>
              )}
            </tbody>
          </table>
                ...
                <tbody className="divide-y">
                  {transactions.map((tx) => (
                    <tr key={tx.id} className="hover:bg-slate-50">
                      <td className="py-2 font-mono text-blue-600">{tx.bookingCode}</td>
                      ...
                      <td className="py-2 text-right font-bold">£{tx.amount.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
